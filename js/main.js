...
const BOSS_BALLOON_RELEASE_DELAY = 9.0; 
...
function getSceneCard(scene, level = state.level || 1) {
    if (scene === "main") {
        const useMotherCard = !!state.motherAcquired ||
            level > 1;
        if (useMotherCard) {
            return (
...
function checkRopeReturn() {
    ...
    const validLandingOrigins = Array.isArray(current.returnFrom) ?
        current.returnFrom : [current.returnTo];
...
function drawCoconutKongBosses() {
    ...
    const shouldDrawStaticBoss = !state.kongIntroSeenThisGame &&
        introPhase === "waiting";
